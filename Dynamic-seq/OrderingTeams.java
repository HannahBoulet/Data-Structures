	...
	public static RBST orderTeams(int N) {
		...
		for (int i = 1; i < N; i++) {
			if (didXBeatY(i, O.select(1).getTeam())) {
				...
				int b = 1;
				int end = O.getSize();
				// testing weather base is less then end -1
				while (b < end - 1) {
					// if it is set a equal to base + end divided by 2
					int a = (b + end) / 2;
					// test if xbeaty with a inserted in select.
					if (didXBeatY(i, O.select(a).getTeam())) {
						// if x did beat y set end equal to a.
						end = a;
					} else {
						// if not set base equal to a.
						b = a;
					}
				}
		...
		if (ordering == null || ordering.getSize() != N) {
			...
			for (int i = 1; i <= ordering.getSize() - 1; i++) {
				...
				if (!didXBeatY(x.getTeam(), y.getTeam())) {
					System.out.println("Invalid sequence: team " + x.getTeam() + " in position " + i + " lost to team "
							+ y.getTeam() + " at position " + (i + 1) + ".");
					System.exit(0);
				}
			}
		}
		System.out.println("Yay! Tree contains a valid ordering.");
		// ordering.print(); You can print the ordreing. But realize that N = 100000, so
		// the ordering will fill your entire screen. Try to set N to something more
		// manageable, like N = 100 and then print it out.
	}
}
